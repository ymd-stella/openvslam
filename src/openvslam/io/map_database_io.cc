...
map_database_io::map_database_io(data::camera_database* cam_db, imu::imu_database* imu_db, data::map_database* map_db,
                                 data::bow_database* bow_db, data::bow_vocabulary* bow_vocab)
    : cam_db_(cam_db), imu_db_(imu_db), map_db_(map_db), bow_db_(bow_db), bow_vocab_(bow_vocab) {}

void map_database_io::save_message_pack(const std::string& path) {
    std::lock_guard<std::mutex> lock(data::map_database::mtx_database_);

    assert(cam_db_ && imu_db_ && map_db_);
    const auto cameras = cam_db_->to_json();
    const auto imus = imu_db_->to_json();
...
void map_database_io::load_message_pack(const std::string& path) {
    ...
    assert(cam_db_ && imu_db_ && map_db_ && bow_db_ && bow_vocab_);
    map_db_->clear();
    bow_db_->clear();
    ...
    if (json.contains("imus")) {
        const auto json_imus = json.at("imus");
        imu_db_->from_json(json_imus);
    }
    const auto json_keyfrms = json.at("keyframes");
    const auto json_landmarks = json.at("landmarks");
    map_db_->from_json(cam_db_, imu_db_, bow_vocab_, bow_db_, json_keyfrms, json_landmarks);
