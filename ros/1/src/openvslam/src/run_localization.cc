...
void mono_localization(const std::shared_ptr<openvslam::config>& cfg, const std::string& vocab_file_path,
    ...
    std::shared_ptr<openvslam::util::dualfisheye2equirectangular> d2e = nullptr;
    ...
    image_transport::Subscriber sub = it.subscribe("camera/image_raw", 1, [&](const sensor_msgs::ImageConstPtr& msg) {
        ...
        if (d2e_) {
            d2e_->convert(image, image);
        }

        // input the current frame and estimate the camera pose
        SLAM.feed_monocular_frame(image, timestamp, mask);
...
int main(int argc, char* argv[]) {
    ...
    if (cfg->camera_->setup_type_ == openvslam::camera::setup_type_t::Monocular) {
        mono_localization(cfg, vocab_file_path->value(), mask_img_path->value(), map_db_path->value(),
                          mapping->is_set(), from_dual_fisheye->is_set());
    }
    else {
