...
TEST(data, io) {
    ...
    imu::internal::inertial_gravity_scale_edge edge;
    ...
    imu::internal::inertial_gravity_scale_edge edge2;
    edge2.read(ss);

    EXPECT_LT(std::abs(edge2.measurement()->dt_ - dT), 1e-6);
    EXPECT_LT((edge2.measurement()->covariance_ - C).cwiseAbs().sum(), 1e-6);
    EXPECT_LT((edge2.measurement()->b_.acc_ - acc).cwiseAbs().sum(), 1e-6);
    EXPECT_LT((edge2.measurement()->b_.gyr_ - gyr).cwiseAbs().sum(), 1e-6);
    EXPECT_LT((edge2.measurement()->jacob_rotation_gyr_ - JRg).cwiseAbs().sum(), 1e-6);
    EXPECT_LT((edge2.measurement()->jacob_velocity_gyr_ - JVg).cwiseAbs().sum(), 1e-6);
    EXPECT_LT((edge2.measurement()->jacob_velocity_acc_ - JVa).cwiseAbs().sum(), 1e-6);
    EXPECT_LT((edge2.measurement()->jacob_position_gyr_ - JPg).cwiseAbs().sum(), 1e-6);
    EXPECT_LT((edge2.measurement()->jacob_position_acc_ - JPa).cwiseAbs().sum(), 1e-6);
    EXPECT_LT((edge2.measurement()->delta_rotation_ - dR).cwiseAbs().sum(), 1e-6);
    EXPECT_LT((edge2.measurement()->delta_velocity_ - dV).cwiseAbs().sum(), 1e-6);
    EXPECT_LT((edge2.measurement()->delta_position_ - dP).cwiseAbs().sum(), 1e-6);
}
